...
- **Dynamic Filename Generation for CSV Output**
  - Both `JForex4Downloader.java` and `JForex4ArchiveAuditor.java` now use `INSTRUMENT.name()` to generate CSV filenames dynamically.
  - Files are now named `SYMBOL_YYYY-MM_1min_OHLCV.csv` (e.g., `GBPUSD_2005-01_1min_OHLCV.csv`, `EURUSD_2005-01_1min_OHLCV.csv`).
  - This eliminates the hardcoded "EURUSD" filename bug that caused data from one instrument to be incorrectly stored in another instrument's CSV files.
  - Archive validation and backfill operations now correctly identify and process files for the active instrument.

- **Enhanced `configure.sh` — Instrument Validation and Auto-Correction**
  - The script now **validates instrument input** before writing to Java files:
    - Rejects entries containing commas or spaces (e.g., `GBPUSD, USDJPY`).
    - Automatically adds the `Instrument.` prefix if the user forgets it (e.g., `GBPUSD` → `Instrument.GBPUSD`).
    - Validates that the instrument name contains only uppercase letters, numbers, and underscores.
  - **Handles invalid current values** — if the existing `CURRENT_INSTRUMENT` is malformed (e.g., from a previous error), the script detects it and forces the user to enter a valid instrument.
  - **Auto-start download after configuration** — after successful configuration and compilation, the script asks:
    ```
